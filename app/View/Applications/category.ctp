<div class="container-fluid">
  ...
  <button type="submit" class="btn">Search</button>
	...
	<? echo $this->Html->link($category['Tag']['name'],array('action'=>'category',$category['Tag']['id'],1), array('class'=>'label_anchor')); ?></span>Number : <? echo $category['count']; ?> <? echo $category['Tag']['description']; ?><Br /><br />
    <? $count++; ?>
	<? endif; ?>
 	<? } ?>   
	<? else : ?>
 ...
 <? foreach($apps as $item){ ?>
  ...
  <? if ($page >1): ?>
  <? echo $this->Html->link('Prev',array('action'=>'category',$id,$page-1)); ?>  
  <? endif ;?>
  <? if ($page > 1 && $page*10 < $total_apps): ?> | <? endif; ?>
  <? if ($page*10 < $total_apps) : ?>
  <? echo $this->Html->link('Next',array('action'=>'category',$id,$page+1)); ?>
  <? endif; ?>
</div>
  <script>
   $('.ratings_stars').hover(  
